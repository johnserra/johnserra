...
### AI Chat Widget
`AIChatWidget.tsx` lazy-loads `AIChatPanel.tsx` on the homepage. `src/app/api/chat/route.ts` and the CLI share the server implementation in `src/lib/chat/`. The assistant answers bounded English and Turkish social messages locally, and uses an evidence-gathering loop with five read-only tools to verify factual draft answers against retrieved evidence. The prompt identifies it as an AI assistant, requires it to refer to John in the third person, and forbids impersonating him. History lives in panel state. See `docs/digital-twin-architecture.md` for the current retrieval and verification flow.
