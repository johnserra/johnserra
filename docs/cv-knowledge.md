...
3. Deploy the CV-aware application/worker code and manually verify that the deployed worker recognizes the strict CV job shape and canonical approval-digest algorithm. Do not enqueue a CV job while an older worker is deployed. During this code-first window, chat falls back only when PostgREST returns `PGRST202` for the absent filtered RPC, using the preserved four-argument WordPress retrieval RPC until migration 00002 exists. The fallback uses the same abort signal and is disabled for explicit document, organization, or role filters because the legacy RPC cannot honor them.
4. Apply additive migration `00002_cv_knowledge.sql`, then verify its service-role-only RPC grants and contract version in the target database. The version RPC proves SQL-contract availability only; it cannot prove which worker code is deployed. Static shell checks are not migration proof.
