...
7. The system instruction combines a hard-coded biography, first-person style rules, an optional Turkish-language instruction, and retrieved context. Generation receives the full supplied history, mapping `user` to Gemini's `user` role and other roles to `model`.
...
| Persona and privacy | First-person persona; contradictory latitude to answer from background knowledge while prohibiting invented facts. Retrieved text is not explicitly isolated as untrusted instructions. No evaluated injection defenses or documented conversation-retention policy. | [#20](https://github.com/johnserra/johnserra/issues/20) |
