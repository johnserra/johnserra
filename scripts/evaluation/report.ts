...
function markdown(report: BaselineReport): string {
  ...
  const lines = [
    ...
    `- Citation expected-source match: ${fraction(live.citationExpectedSourceMatch)} (all expected canonical source paths cited without unexpected johnserra.com paths; normalized path match only, runtime HTTP resolution not performed)`,
  ...
  for (const result of report.cases) {
    ...
    lines.push(
      ...
      "",
      `Citation URLs/matches: ${score.citations.urls.join(", ") || "none"} / ${score.citations.matchingUrls.join(", ") || "none"}${score.citations.unexpectedJohnSerraPaths?.length ? ` (unexpected: ${score.citations.unexpectedJohnSerraPaths.join(", ")})` : ""}${score.citations.missingExpectedPaths?.length ? ` (missing expected: ${score.citations.missingExpectedPaths.join(", ")})` : ""}`,
      "",
